...
export const AddNewEmailModal = ({
  ...
  const [selectedDashboards, setSelectedDashboards] = useState<Set<string>>(new Set(["software"]));
  ...
  // Ensure Software Dashboard is selected when modal opens
  useEffect(() => {
    if (isOpen) {
      setSelectedDashboards(new Set(["software"]));
    }
  }, [isOpen]);
            ...
            <div className={`flex items-center justify-between px-6 pt-6 pb-4 flex-shrink-0 rounded-t-2xl ${
              ...
              <div className="flex items-center gap-3">
                <div className={`flex h-9 w-9 items-center justify-center rounded-full ${tokens.isDark ? tokens.buttonGhost : ""}`} style={tokens.isDark ? {} : { backgroundColor: "#E6E9FB" }}>
                  <EmailIcon className={`h-5 w-5`} style={tokens.isDark ? {} : { color: "#071FD7" }} />
                </div>
            ...
            <div className={`flex flex-col gap-3 mt-6 flex-shrink-0 border-t mx-6 ${tokens.isDark ? "border-white/20" : "border-[#E6E6E6]"} pt-2 pb-6`}>
              <button
                ...
                className={`w-full py-3.5 rounded-full font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                  tokens.isDark
                    ? "bg-[#0F1217] text-white hover:bg-[#0F1217]/90"
                    : "bg-[#FFFEF7] text-black hover:bg-[#FFFEF7]/90"
                }`}
              >
                Add
              </button>
