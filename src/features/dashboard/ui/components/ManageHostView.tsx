...
export const ManageHostView = ({ hostId, tokens }: ManageHostViewProps) => {
  ...
  const usageMetrics = useMemo(
    () =>
      [
        ...
        { id: "databases", label: "Databases Used", used: "4 / 10", total: "15.2 GB / 50 GB", percent: 40, Icon: DatabaseIcon }
      ] as const,
    []
  );
  ...
  const recentTickets = useMemo(
    () =>
      [
        ...
        { id: "2408", title: "Domain setup", status: "In Progress" as const }
      ] as const,
    []
  );
  ...
  const getStatusColor = (status: string) => {
    if (status === "Active" || status === "Resolved") {
      return "bg-[#F4FFF7] text-[#73F593]";
    }
    if (status === "Pending" || status === "In Progress") {
      return "bg-[#FFFBF4] text-amber-500";
    }
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-center border-b border-dashed border-[#A3AED0] pb-6">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className={`${tokens.subtleText} text-sm`}>Shared Hosting</span>
            </div>
          </div>
          <span
      ...
      <div className={`${tokens.cardBase} ${cardClass}`}>
        ...
        <div className="flex flex-col gap-3">
          <p className={`${tokens.subtleText} text-sm font-semibold uppercase mb-2`}>Recent Tickets</p>
          {recentTickets.map((ticket) => (
            <div
              key={ticket.id}
              className="flex items-center justify-between rounded-2xl px-4 py-3 bg-[var(--color-table-row-bg)]"
            >
              <div className="flex items-center gap-3">
                <TicketIcon className={`h-5 w-5 ${tokens.isDark ? "" : "text-[#584ABC]"}`} />
                <div>
                ...
                <tr key={row.domain}>
                  ...
                  <td className="rounded-r-2xl px-4 py-2">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      >
                        <ArrowRightIcon className={`h-4 w-4 ${tokens.isDark ? "" : "[&_path]:stroke-[#584ABC]"}`} />
                      </button>
                      <button
                        type="button"
                        className={`${tokens.buttonGhost} flex h-9 w-9 items-center justify-center rounded-full`}
                      ...
                      >
                        <DatabaseIcon className={`h-4 w-4 ${tokens.isDark ? "" : "[&_path]:fill-[#584ABC]"}`} />
                      </button>
                    </div>
                  </td>
                </tr>
